...
export function AvailabilityPicker({
  ...
  const [selected, setSelected] = useState(() => new Set(initialSlots.map(slot => slot.slot_start)));
  ...
  async function save() {
    ...
    const rows = Array.from(selected).map(start => ({
      owner_type: ownerType,
      owner_id: ownerId,
      slot_start: start,
      slot_end: new Date(new Date(start).getTime() + 30 * 60 * 1000).toISOString()
    }));
    await supabase.from("availability_slots").delete().eq("owner_type", ownerType).eq("owner_id", ownerId);
    const { error } = rows.length ? await supabase.from("availability_slots").insert(rows) : { error: null };
    setSaving(false);
    setMessage(error ? error.message : "时间已保存");
  }
  ...
  return (
    <section className="panel">
      <div className="weekbar">
        {weeks.map(week => (
          <button key={week.index} className={week.index === weekIndex ? "active" : ""} onClick={() => setWeekIndex(week.index)}>
      ...
      <div className="grid-scroll">
        <div className="schedule-grid">
          ...
          {activeWeek.days.map(day => <div className="cell head" key={day.iso}>{day.label}</div>)}
          {slotTimes.flatMap(time => [
            <div className="cell time" key={`time-${time}`}>{time}</div>,
            ...activeWeek.days.map(day => {
              const active = hasSlot(day.iso, time);
              return (
                <div className="cell" key={`${day.iso}-${time}`}>
                  <button className={`slot ${active ? "selected" : ""}`} onClick={() => toggle(day.iso, time)}>
                    {active ? "可用" : ""}
                  </button>
                </div>
              );
